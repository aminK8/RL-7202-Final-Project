...
aesthetic_model = Aesthetic_MLP(768)
weight = torch.load("aesthetic/sac+logos+ava1-l14-linearMSE.pth") 
aesthetic_model.load_state_dict(weight)
aesthetic_model.eval()
aesthetic_model.to("cuda:0")
  ...
  for _ in range(args.g_step):
    ...
    with torch.no_grad():
      (
          ...
          log_prob_list,
          _,
      ) = pipe.forward_collect_traj_ddim(prompt=batch)
      reward_list = []
      txt_emb_list = []
      for i in range(len(batch)):
        _, txt_emb = calculate_reward(image[i], batch[i])
        reward = calculate_image_reward(image[i])
...
def calculate_image_reward(image: Image):
    pil_image = Image.fromarray((image * 255.0).astype(np.uint8))
    image = preprocess(pil_image).unsqueeze(0).to("cuda:0")
    with torch.no_grad():
        image_features = clip_model.encode_image(image)
    image_features = image_features.cpu().detach().numpy()
    l2 = np.atleast_1d(np.linalg.norm(image_features, 2, -1))
    l2[l2 == 0] = 1
    im_emb_arr = image_features / np.expand_dims(l2, -1)
    im_emb_arr = torch.from_numpy(im_emb_arr).to("cuda:0").type(torch.cuda.FloatTensor)
    reward = aesthetic_model(im_emb_arr).item()
    return reward
    ...
    if args.v_flag == 1:
      ...
      if accelerator.is_main_process:
        ...
        with open("output.txt", "a") as file:
            file.write(f"epoch: {count}:\n")
            file.write(f" value_loss: {tot_val_loss}\n")
      del tot_val_loss
      torch.cuda.empty_cache()

    # policy learning
    tpfdata = TrainPolicyFuncData()
    for step in range(args.p_step):
      optimizer.zero_grad()
      ...
      if accelerator.is_main_process:
        with open("output.txt", "a") as file:
          file.write(f" policy step {step}:\n")
          file.write(f"  policy loss: {tpfdata.tot_p_loss}\n")
          file.write(f"  grad norm: {tpfdata.tot_grad_norm}\n")
          file.write(f"  train_reward: {torch.mean(state_dict['final_reward']).item()}\n")
