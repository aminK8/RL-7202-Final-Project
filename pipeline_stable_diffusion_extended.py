...
class StableDiffusionPipelineExtended(StableDiffusionPipeline):
    ...
    def forward_collect_traj_ddim(
        ...
        Args:
            ...
            soft_reward (`bool`, *optional*, defaults to `False`): whether to use
              soft reward
            show_progress (bool), optional: whether to show the tqdm progress bar 
        ...
        Returns:
            ...
            each step
        """
        self.set_progress_bar_config(disable=not show_progress)
        ...
        self.scheduler.final_alpha_cumprod = self.scheduler.final_alpha_cumprod.to(
            device
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if soft_reward:
                    unet_times = unsqueeze3x(
                        torch.tensor([t] * noise_pred.shape[0])
    ...
    def forward_calculate_logprob(
        ...
        time_model_input = (
            torch.cat([unet_times] * 2) if do_classifier_free_guidance else unet_times
        )
