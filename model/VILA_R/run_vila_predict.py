...
import models.VILA_R.coca_vila as coca_vila
import models.VILA_R.coca_vila_configs as coca_vila_configs


NestedMap = py_utils.NestedMap

_CKPT_DIR = "./models/VILA_R/checkpoints/vila_rank_tuned"
_SPM_MODEL_PATH = "./models/VILA_R/spm_model/spm.model"
...
def preprocess_image(
    image: np.ndarray, pre_crop_size: int, image_size: int
):
  """Image preprocessing."""
...
def predict_aesthetic_score(image: np.ndarray)-> Tuple[float, np.ndarray]:
    # Suppresses verbose INFO/DEBUG log.
    logging.set_verbosity(logging.ERROR)
    model, model_states = load_vila_model(_CKPT_DIR)
    image = preprocess_image(image, _PRE_CROP_SIZE, _IMAGE_SIZE)
    ...
    if _SPM_MODEL_PATH:
        ...
        with base_layer.JaxContext.new_context(hparams=context_p):
            ...
            # convert to numpy 
            text_embed_arr = jax.device_get(text_embed)
            zsl_scores = zsl_scores.item()
    return zsl_scores, text_embed_arr

if __name__ == '__main__':
  image = cv2.imread("forest.jpg")
  image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
  print(predict_aesthetic_score(image))